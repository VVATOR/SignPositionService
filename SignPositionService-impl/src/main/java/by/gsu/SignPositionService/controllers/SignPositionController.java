...
//@Controller
//@RequestMapping("/")
public class SignPositionController implements SEISign {

    public static final Logger LOG = LoggerFactory.getLogger(SignPositionController.class);

    private static AtomicLong id = new AtomicLong(10);
    private static List<Sign> store = new CopyOnWriteArrayList<Sign>() {
        {
            add(new Sign(1));
 ...
 //   @RequestMapping(value = "sign", method = RequestMethod.GET)
    public ResponseEntity<List<Sign>> methodGetListSigns(HttpServletResponse response) {
 ...
 //   @RequestMapping(value = "sign", method = RequestMethod.PUT)
    public ResponseEntity<Long> methodPutSign(@RequestParam Sign sign, final HttpServletResponse response) {
        ...
        // public ResponseEntity<Void> methodPutSign(Sign sign, HttpServletResponse response) {
        /*
         * for (int index = 0; index < store.size(); index++) { if (store.get(index).getId() == sign.getId()) { store.set(index, sign); return (ResponseEntity<Void>) ResponseEntity.ok(); } } return (ResponseEntity<Void>)
